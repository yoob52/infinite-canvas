...
启动后访问 `http://localhost:3000/workbench/`，根路径 `/` 会跳转到该地址。
...
如果域名根路径已经指向中转站，把本容器挂到 `/workbench` 即可：
...
`proxy_pass` 不要去掉 `/workbench` 前缀。画布里的接口 Base URL 填 `https://xxx.abc.com`（和中转 `/v1` 同源），不要填 `/workbench`。
